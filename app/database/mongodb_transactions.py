...
def session_transaction(collection, user_id, username, login_time):
    user_logins_collection = mongo1_db[f'{collection}']
    session = mongo1_client.start_session()
    with session.start_transaction():
        try:
            user_logins_collection.insert_one({'user_id': user_id, 'username': username, 'login_time': login_time}, session=session)
